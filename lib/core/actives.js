...
const addPiniaAction = async (name, dest) => {
  const piniaResult = await compile('pinia.ejs', {
    name,
    UpperName: firstToUp(name),
  })

  if (createDirSync(dest)) {
    const piniaPath = path.resolve(dest, `${name}.js`)
    writeToFile(piniaPath, piniaResult)
  }
}
